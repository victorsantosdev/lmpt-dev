...
const uint8_t TERM_PLACA = A1;
const uint8_t TERM_AR = A2;
...
void loop() {
    ...
    if ((tempo_ms - lcd_serial_cadencia_ms) >= LCD_SERIAL_CADENCIA) {
        ...
        escreveTensao_lcd(ads1248_ad[1], 0);
        escreveTensao_lcd(ads1248_ad[2], 1);
 ...
 *
 */
void enviaSerial(uint8_t sensorType, uint8_t sensorNumber, uint8_t sensorData) {
    packet serialOut;
...
//função recebe como argumento o erro a ser inserido no controlador e responde em numero inteiro 0-4096 para aplicar direto no DAC de 12 bits
